...
class BaseLinlayout : LinearLayout {
    ...
    private fun initView(context: Context, attrs: AttributeSet?) {
        if (attrs != null) {
            val typedArray = context.obtainStyledAttributes(attrs, R.styleable.DrawerLinlayout)
            if (typedArray != null) {
                title = typedArray.getString(R.styleable.DrawerLinlayout_drawer_title)
                version = typedArray.getString(R.styleable.DrawerLinlayout_drawer_title)
                leftIcon = typedArray.getResourceId(
                    R.styleable.DrawerLinlayout_drawer_image_left,
                    R.drawable.ic_setting
                )
                rightIcon = typedArray.getResourceId(
                    R.styleable.DrawerLinlayout_drawer_image_right,
